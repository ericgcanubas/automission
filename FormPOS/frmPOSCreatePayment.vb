...
Public Class frmPOSCreatePayment
    ...
    Private Sub fCompute()
        ...
        For N As Integer = 0 To dgvAVAILABLE.Rows.Count - 1
            NS = NS + fNumisNULL(dgvAVAILABLE.Rows(N).Cells("AMOUNT").Value)
        Next
        xlblAV_AMOUNT.Text = fNumFormatStandard(NS)

        For I As Integer = 0 To dgvSELECTED.Rows.Count - 1
            A = A + fNumisNULL(dgvSELECTED.Rows(I).Cells("AMOUNT_APPLIED").Value)
        Next
        lblAMOUNT_APPLIED.Text = fNumFormatStandard(A)
    ...
    Private Sub fAvailable_Load()
        ...
        Dim rd As OdbcDataReader = fReader($"SELECT s.ID,s.CODE, s.AMOUNT FROM invoice as S  WHERE  s.CUSTOMER_ID ='{gsCUSTOMER_ID}' and s.LOCATION_ID ='{gsDefault_LOCATION_ID}' and s.SHIP_TO ='{gsTABLE_NO}'  and s.SHIP_VIA_ID='{gsORDER_TYPE}' and s.STATUS in('13')")
        While rd.Read
    ...
    Private Sub fGotAmount_Applied(ByVal INVOICE_ID As Integer, ByRef BALANCE As Double)
        ...
        Dim rd As OdbcDataReader = fReader($"select sum(AMOUNT_APPLIED) as `PAY` from payment_invoices WHERE INVOICE_ID = '{INVOICE_ID}' ")
        If rd.Read Then
            PAY_AMT = fNumisNULL(rd("PAY"))
        End If
        rd.Close()
    ...
    Private Sub BtnAddAll_Click(sender As Object, e As EventArgs) Handles btnAddAll.Click
        ...
        For S As Integer = 0 To dgvAVAILABLE.Rows.Count - 1
            ...
            Dim BALANCE As Double = fNumisNULL(A.Cells(2).Value)
    ...
    Private Function fGetRemaining_Deposit() As Double
        ...
        For I As Integer = 0 To dgvSELECTED.Rows.Count - 1
            D = D - fNumisNULL(dgvSELECTED.Rows(I).Cells("AMOUNT_APPLIED").Value)
        Next
    ...
    Private Sub BtnOK_Click(sender As Object, e As EventArgs) Handles btnOK.Click
        ...
        If numAMOUNT.Value = 0 Then
            ...
            If fGo4Bundle() = False Then
                fMessageboxInfo("Please enter tender amount")
                Exit Sub
            Else
                If fMessagePOSYesNO("Do you want to served this bundle?") = False Then
                    Exit Sub
                Else
                    gsBundle = True
                End If
        ...
        If dgvSELECTED.Rows.Count = 0 Then
            fMessageboxInfo("No served selected.")
            Exit Sub

        End If

        If dgvAVAILABLE.Rows.Count > 1 And fGo4Bundle() = False Then
            If fMessagePOSYesNO("Are you sure create this payment?") = False Then
                Exit Sub
            End If
        End If
...
SET `ID` = '{ThisID}',
  `PAYMENT_ID` = '{gsID}',
  `INVOICE_ID` = '{ fNumisNULL(dgvSELECTED.Rows(I).Cells("ID").Value)}',
  `DISCOUNT` = NULL,
  `AMOUNT_APPLIED` = '{fNumisNULL(dgvSELECTED.Rows(I).Cells("AMOUNT_APPLIED").Value)}',
  `DISCOUNT_ACCOUNT_ID` = NULL,
  `ACCOUNTS_RECEIVABLE_ID` = '{gsDefault_ACCOUNTS_RECEIVABLE_ID}',
  `PENALTY_PAID` = NULL;")

            fUpdateInvoiceBalance(fNumisNULL(dgvSELECTED.Rows(I).Cells("ID").Value), gsCUSTOMER_ID)
            '======================================================
            If gsSkipJournalEntry = False Then
                fAccount_Journal_SQL(gsDefault_ACCOUNTS_RECEIVABLE_ID, gsDefault_LOCATION_ID, fNumisNULL(dgvSELECTED.Rows(I).Cells("ID").Value), 42, ThisID, gsPOS_DATE, 1, fNumisNULL(dgvSELECTED.Rows(I).Cells("AMOUNT_APPLIED").Value), gsJOURNAL_NO_FORM)
            End If
            '=====================================================
        Next
    ...
    Private Sub CmbPAYMENT_METHOD_ID_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbPAYMENT_METHOD_ID.SelectedIndexChanged
        Try
            ...
            Dim I As Integer = fNumisNULL(cmbPAYMENT_METHOD_ID.SelectedValue)
