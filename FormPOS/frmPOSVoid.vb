...
Public Class FrmPOSVoid
    ...
    Private Sub fPOS_Payment()
        fDataGridView(dgvPAYMENT, $"SELECT P.ID,P.CODE as `REF No.`,P.RECEIPT_REF_NO as `OR No.`, PN.AMOUNT_APPLIED as `PAID` FROM PAYMENT as P INNER JOIN PAYMENT_INVOICES AS PN on PN.PAYMENT_ID = P.ID  WHERE P.LOCATION_ID = '{gsDefault_LOCATION_ID}' and P.POS_LOG_ID = '{gsPOS_LOG_ID}' ")
    ...
    Private Sub fPOS_SUM()
        ...
        For N As Integer = 0 To dgvPAYMENT.Rows.Count - 1
            P = P + fNumisNULL(dgvPAYMENT.Rows(N).Cells("PAID").Value)
        Next
        lblSales.Text = fNumFormatStandard(P)
    ...
    Private Sub fSelected()
        ...
        If dgvPAYMENT.Rows.Count <> 0 Then
            ...
            With dgvPAYMENT.CurrentRow
                ...
                lblPAID.Text = fNumFormatStandard(.Cells("PAID").Value)
            End With
        ...
        fDataGridView(dgvITEM, $"select ii.id,ii.ITEM_ID,i.DESCRIPTION,ii.QUANTITY as `QTY`,ii.RATE,ii.AMOUNT from payment_invoices as pn inner join invoice_items as ii  on ii.invoice_id = pn.invoice_id inner join item as i on i.id = ii.item_id  where ii.PRINT_IN_FORMS = '0' and  pn.payment_id = '{ID}' limit 100")
    ...
    Private Sub btnDELETE_Click(sender As Object, e As EventArgs) Handles btnDELETE.Click
        ...
        If dgvITEM.Rows.Count <> 0 Then
            ...
            With dgvPAYMENT.CurrentRow
                Dim ID As Integer = fNumisNULL(.Cells(0).Value)
                If fMessagePOSYesNO("Do you want to delete this record?") = True Then
                    fPayment_Delete(ID)
                End If
            End With
    ...
    Private Sub fMustPrint(ByVal ThisID As Integer)
        If fMessagePOSYesNO($"Do you want to Print?") = True Then
    ...
    Private Sub fPayment_Delete(ByVal prID As Integer)
        ...
        Dim rd As OdbcDataReader = fReader($"select * from payment_invoices where payment_id = '{prID}'")
        If rd.Read Then
            ...
            If gsPOSVoidEntry = False Then
                ...
                fExecutedOnly("DELETE FROM payment_invoices WHERE ID='" & rd("ID") & "' and PAYMENT_ID='" & prID & "' and INVOICE_ID='" & Invoice_ID & "'")
        ...
        If gsPOSVoidEntry = False Then
            fExecutedOnly("DELETE FROM payment WHERE ID = '" & prID & "' limit 1;")

            fTransaction_Log(prID, lblPAYMENT_CODE.Text, 3, "Delete", gsPOSDefaultCustomer_ID, "", fNumisNULL(lblPAID.Text), gsDefault_LOCATION_ID)

        Else
            fExecutedOnly("UPDATE payment SET STATUS ='7' WHERE ID = '" & prID & "' limit 1;")

            fTransaction_Log(prID, lblPAYMENT_CODE.Text, 3, "Void", gsPOSDefaultCustomer_ID, "", fNumisNULL(lblPAID.Text), gsDefault_LOCATION_ID)
    ...
    Private Sub fInvoiceVoid(ByVal Invoice_ID As Integer)
        If Invoice_ID <> 0 Then
            ...
            Dim r As OdbcDataReader = fReader($"select * from invoice where id ='{Invoice_ID}' limit 1")
            If r.Read() Then
                DT = CDate(r("DATE"))
                Loc_ID = fNumisNULL(r("LOCATION_ID"))
                INVOICE_CODE = fNumisNULL(r("CODE"))
            End If
            r.Close()
            ...
            If gsSkipJournalEntry = False Then
                gsJOURNAL_NO_FORM = 0
                Dim Account_ID As Integer = fNumFieldValue("INVOICE", "ID", Invoice_ID, "ACCOUNTS_RECEIVABLE_ID")
                Dim Tax_Acct_ID As Integer = fNumFieldValue("INVOICE", "ID", Invoice_ID, "OUTPUT_TAX_ACCOUNT_ID")
            ...
            If gsPOSVoidEntry = True Then
                fExecutedOnly("UPDATE invoice SET STATUS ='7' where `ID` ='" & Invoice_ID & "' limit 1;")
                fTransaction_Log(Invoice_ID, INVOICE_CODE, 2, "Void", Loc_ID, "", fNumisNULL(lblPAID.Text), gsDefault_LOCATION_ID)

            Else
                'Delete
                fExecutedOnly("DELETE from invoice_items where INVOICE_ID  = '" & Invoice_ID & "'")
                fExecutedOnly("DELETE from invoice where `ID` ='" & Invoice_ID & "' limit 1;")
                fTransaction_Log(Invoice_ID, INVOICE_CODE, 2, "Delete", Loc_ID, "", fNumisNULL(lblPAID.Text), gsDefault_LOCATION_ID)
    ...
    Private Sub fDeletePOSVoid_Invoice_Item(ByVal INVOICE_ID As Integer, ByVal DT As Date, ByVal Loc_ID As Integer)
        ...
        Dim rd As OdbcDataReader = fReader($"select n.id,n.item_id,i.type,n.ASSET_ACCOUNT_ID,n.COGS_ACCOUNT_ID from invoice_items as n inner join item as i on i.id = n.item_id where n.invoice_ID ='{INVOICE_ID}'  limit 1;")
        ...
        While rd.Read
            ...
            If gsSkipJournalEntry = False Then
                fAccount_journal_Delete(fNumisNULL(rd("INCOME_ACCOUNT_ID")), Loc_ID, 24, rd("id"), DT)

                If rd("type") = 0 Or rd("type") = 1 Then
                    'ITEM INVENTORY/ ASSEMBLY
                    fAccount_journal_Delete(fNumisNULL(rd("ASSET_ACCOUNT_ID")), Loc_ID, 24, rd("id"), DT)
                    fAccount_journal_Delete(fNumisNULL(rd("COGS_ACCOUNT_ID")), Loc_ID, 24, rd("id"), DT)
                End If
            End If
        End While
    ...
    Private Sub fRecaluculateItemInvoie(ByVal Invoice_ID As Integer, ByVal Loc_ID As Integer, ByVal dt As Date)
        ...
        Dim rd As OdbcDataReader = fReader($"select * from invoice_items where invoice_id = '{Invoice_ID}' ")
        While rd.Read
            fReCalculateInventory(Invoice_ID, Loc_ID, dt.AddDays(-1))
        End While
        rd.Close()
    End Sub
End Class