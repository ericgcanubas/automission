Public Class FrmPOSRoomEntryList
    ...
    Private Sub fEntryList()
        ...
        fDataGridView(dgvList, $"select i.ID, ifnull( {strQuery},0) as `ROOM_ID`,i.CODE as `Billing No.`, format(i.AMOUNT,2) as `TOTAL`, format(i.BALANCE_DUE,2) as `BALANCE`,s.DESCRIPTION as `Status`  from invoice as i inner join ship_via as s on s.id = i.SHIP_VIA_ID where i.manager_id = '{gsPOS_LOG_ID}' order by i.id ")
