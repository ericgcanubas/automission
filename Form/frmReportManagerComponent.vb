Public Class FrmReportManagerComponent
    ...
    Private Sub Save_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If ID = 0 Then
            ...
            ID = GF_GetMaxField("ID", "report_components")
            ...
            ID = 0
            LoadComponents()
            numLINE_NO.Value = Val(GF_GetMaxFieldLine("LINE_NO", "report_components", "SUB_ID", SUB_ID))
        Else
    ...
    Private Sub LoadComponents()
        If ID = 0 Then
            GS_ComboBoxLoad(cmbCOMPONENT_ID, "SELECT o.ID,o.NAME FROM COMPONENT AS o WHERE NOT EXISTS (SELECT c.ID,c.NAME FROM component AS c INNER JOIN report_components AS r ON r.COMPONENT_ID = c.ID WHERE o.ID = c.`ID` and r.SUB_ID = '" & SUB_ID & "')", "ID", "NAME")
        Else
            GS_ComboBoxLoad(cmbCOMPONENT_ID, "SELECT o.ID,o.NAME FROM COMPONENT AS o WHERE NOT EXISTS (SELECT c.ID,c.NAME FROM component AS c INNER JOIN report_components AS r ON r.COMPONENT_ID = c.ID WHERE o.ID = c.`ID` and  r.ID <> '" & ID & "' and r.SUB_ID = '" & SUB_ID & "')", "ID", "NAME")
        End If
    End Sub
End Class