...
Public Class FrmWriteCheck
    ...
    Private Sub ComboBoxRefresh()
        ...
        GS_ComboBoxLoad(cmbBANK_ACCOUNT_ID, "SELECT a.ID,a.`NAME` AS `BANK`  FROM account AS a WHERE a.`type` IN ('0', '6')", "ID", "BANK")
        'GS_ComboBoxLoad(cmbPAY_TO_ID, "SELECT c.`ID`, CONCAT(c.`NAME`,' / ',ctm.`DESCRIPTION` ) AS `PAY_TO` FROM contact AS c INNER JOIN  contact_type_map AS ctm ON ctm.`ID` = c.`TYPE` WHERE  c.TYPE = '0' and  c.`INACTIVE` ='0' ORDER BY c.`TYPE`", "ID", "PAY_TO")
        GS_ComboBoxLoad(cmbPAY_TO_ID, "SELECT c.`ID`, c.`NAME` AS `PAY_TO` FROM contact AS c INNER JOIN  contact_type_map AS ctm ON ctm.`ID` = c.`TYPE` WHERE c.`INACTIVE` ='0' ORDER BY c.TYPE,c.NAME ", "ID", "PAY_TO")
  ...
  LEFT OUTER JOIN account AS a 
    ...
    Private Sub EditItem()
        Try
            ...
            With FrmAddItem
                If .gsSave = True Then
                    GS_RowDataItemWriteCheck(dgvProductItem, False, .gsItem_ID, .gsQty, .gsUnit_Price, .cmbDiscount_Type.Text, .gsDiscount_Rate, .gsAmount, .gsTax, .cmbUM.SelectedValue, "E", .gsBase_Qty, .gsDiscount_Type, .gsOriginal_Amount, .gsSelection_ID, .gsBATCH_ID)
                End If
            End With
            Computed()
            FrmAddItem.Dispose()
    ...
    Private Sub TsSaveNew_Click(sender As Object, e As EventArgs) Handles tsSaveNew.Click
        ...
        If IsNew = True Then
            ...
            SetTransactionLog(ID, txtCODE.Text, Me.AccessibleName, "New", cmbPAY_TO_ID.SelectedValue, "", GF_NumIsNull(lblAMOUNT.Text), cmbLOCATION_ID.SelectedValue)

        Else
            If GS_IsInventoryLastEntry(dgvProductItem, cmbLOCATION_ID.SelectedValue, 21, dtpDATE.Value) = False Then
                Exit Sub
            End If
            ...
            SetTransactionLog(ID, txtCODE.Text, Me.AccessibleName, "Edit", cmbPAY_TO_ID.SelectedValue, "", GF_NumIsNull(lblAMOUNT.Text), cmbLOCATION_ID.SelectedValue)
        ...
        If gsSkipJournalEntry = False Then
            ...
            If gsGotChangeDate = True Then
                ...
                'TAX
                AccountJournalChangeDate(dtpDATE.Value, GF_NumIsNull(lblINPUT_TAX_ACCOUNT_ID.Text), 57, ID, gsLast_Location_ID, gsLast_Date)
            End If

            If gsGotChangeLocation1 = True Then
                'Main
                AccountJournalChangeLocation(cmbLOCATION_ID.SelectedValue, GF_NumIsNull(cmbACCOUNTS_PAYABLE_ID.SelectedValue), 57, ID, dtpDATE.Value, gsLast_Location_ID)
                'TAX
                AccountJournalChangeLocation(cmbLOCATION_ID.SelectedValue, GF_NumIsNull(lblINPUT_TAX_ACCOUNT_ID.Text), 57, ID, dtpDATE.Value, gsLast_Location_ID)
            End If
    ...
    Private Sub InventorySetUpdate()
        For I As Integer = 0 To dgvProductItem.Rows.Count - 1
            With dgvProductItem.Rows(I)
                GS_UpdateItemInventory_AccountJournalCost(.Cells("ITEM_ID").Value, cmbLOCATION_ID.SelectedValue, .Cells("ID").Value, 21, 75)
            End With
        Next
    End Sub
    Private Sub SetNew()
            ...
            With dgvProductItem.Rows(i)
                Select Case .Cells("CONTROL_STATUS").Value
                    ...
                    Case "S"
                        ...
                        If gsSkipJournalEntry = False Then
                            If gsGotChangeDate = True Then
                                AccountJournalChangeDate(dtpDATE.Value, .Cells("ACCOUNT_ID").Value, 75, GF_NumIsNull(.Cells("ID").Value), gsLast_Location_ID, gsLast_Date)
                            End If
                            If gsGotChangeLocation1 = True Then
                                AccountJournalChangeLocation(cmbLOCATION_ID.SelectedValue, .Cells("ACCOUNT_ID").Value, 75, GF_NumIsNull(.Cells("ID").Value), dtpDATE.Value, gsLast_Location_ID)
                            End If
                        End If

                        If gsGotChangeDate = True Then
                            ItemInventoryChangeDate(dtpDATE.Value, .Cells("ITEM_ID").Value, 21, GF_NumIsNull(.Cells("ID").Value), gsLast_Location_ID, gsLast_Date)
                        End If

                        If gsGotChangeLocation1 = True Then
                            ItemInventoryChangeLocation(cmbLOCATION_ID.SelectedValue, .Cells("ITEM_ID").Value, 21, GF_NumIsNull(.Cells("ID").Value), dtpDATE.Value, gsLast_Location_ID)
                        End If
                    ...
                    Case "D"
                        SqlExecuted("DELETE FROM check_items WHERE CHECK_ID ='" & ID & "' and ID = '" & GF_NumIsNull(.Cells("ID").Value) & "' limit 1;")
                        GS_InventoryJournalVendors(dgvProductItem, i, False, 75, 21, cmbLOCATION_ID.SelectedValue, dtpDATE.Value, GF_NumIsNull(.Cells("UNIT_PRICE").Value))
            ...
            With dgvExpenses.Rows(i)
                ...
                Select Case .Cells("CONTROL_STATUS").Value
                    ...
                    Case "S"
                        ...
                        If gsSkipJournalEntry = False Then
                            If gsGotChangeDate = True Then
                                AccountJournalChangeDate(dtpDATE.Value, GF_NumIsNull(.Cells("Account_ID").Value), 79, GF_NumIsNull(.Cells("ID").Value), gsLast_Location_ID, gsLast_Date)
                            End If
                            If gsGotChangeLocation1 = True Then
                                AccountJournalChangeLocation(cmbLOCATION_ID.SelectedValue, GF_NumIsNull(.Cells("Account_ID").Value), 79, GF_NumIsNull(.Cells("ID").Value), dtpDATE.Value, gsLast_Location_ID)
                            End If
                    ...
                    Case "D"
                        ...
                        SqlExecuted("DELETE FROM check_expenses WHERE CHECK_ID ='" & ID & "' and ID = '" & GF_NumIsNull(.Cells("ID").Value) & "' limit 1;")
                        If gsSkipJournalEntry = False Then
                            GS_AccountJournalDelete(GF_NumIsNull(.Cells("Account_ID").Value), cmbLOCATION_ID.SelectedValue, 79, .Cells("ID").Value, dtpDATE.Value)
                        End If
                End Select
            End With
        Next
        ...
        If IsNew = False Then
            ...
            If MessageBoxQuestion(gsMessageQuestion) = True Then
                ...
                If gsSkipJournalEntry = False Then
                    ...
                    If GF_NumIsNull(lblINPUT_TAX_AMOUNT.Text) <> 0 Then
                        GS_AccountJournalDelete(Val(lblINPUT_TAX_ACCOUNT_ID.Text), cmbLOCATION_ID.SelectedValue, 57, ID, dtpDATE.Value)
                    End If
                End If
                ...
                DeleteNotify(Me)
                SetTransactionLog(ID, txtCODE.Text, Me.AccessibleName, "Delete", cmbPAY_TO_ID.SelectedValue, "", GF_NumIsNull(lblAMOUNT.Text), cmbLOCATION_ID.SelectedValue)
            ...
            If cmbPAY_TO_ID.SelectedIndex = -1 Then
                ...
                If IsNew = True Then
                    ...
                    If FrmContactDetails.gsOK = True Then
                        GS_ComboBoxLoad(cmbPAY_TO_ID, "select c.id, c.`NAME` from contact as  c  where c.`type` in ('0') and c.inactive = '0' order by c.`NAME` ", "ID", "NAME")
    ...
    Private Sub TsAddItem_Click(sender As Object, e As EventArgs) Handles tsAddItem.Click
        If GF_NumIsNull(cmbBANK_ACCOUNT_ID.SelectedValue) = 0 Then
            MessageBoxInfo("Select bank account")
            Exit Sub
        End If

        If GF_NumIsNull(cmbPAY_TO_ID.SelectedValue) = 0 Then
