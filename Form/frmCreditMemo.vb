...
ON B.ID = II.BATCH_ID
    ...
    Private Sub tsSaveNew_Click(sender As Object, e As EventArgs) Handles tsSaveNew.Click
        ...
        If IsNew = True Then
            ...
            If Trim(txtCODE.Text) = "" Then
                ...
                txtCODE.Text = GetNextCode("CREDIT_MEMO", cmbLOCATION_ID.SelectedValue)
            End If
        ...
        If gsSkipJournalEntry = False Then
            ...
            fAccount_Journal_SQL(Val(cmbACCOUNTS_RECEIVABLE_ID.SelectedValue), cmbLOCATION_ID.SelectedValue, cmbCUSTOMER_ID.SelectedValue, 12, ID, dtpDATE.Value, 1, NumIsNull(lblAMOUNT.Text), gsJOURNAL_NO_FORM)
            ...
            Else
                fAccount_Journal_SQL(Val(lblOUTPUT_TAX_ACCOUNT_ID.Text), cmbLOCATION_ID.SelectedValue, cmbCUSTOMER_ID.SelectedValue, 12, ID, dtpDATE.Value, 0, NumIsNull(lblOUTPUT_TAX_AMOUNT.Text), gsJOURNAL_NO_FORM)
            End If
        ...
        If IsTransactionSuccess(ID, "CREDIT_MEMO") = False Then
            ...
            With dgvProductItem.Rows(i)
                Select Case .Cells("CONTROL_STATUS").Value
                    Case "S"
                        ...
                        SqlExecuted("UPDATE credit_memo_items SET LINE_NO='" & i & "',TAXABLE_AMOUNT = '" & NumIsNull(.Cells("TAXABLE_AMOUNT").Value) & "',TAX_AMOUNT='" & NumIsNull(.Cells("TAX_AMOUNT").Value) & "' WHERE CREDIT_MEMO_ID ='" & ID & "' and ID = " & GotNullNumber(NumIsNull(.Cells("ID").Value)) & " limit 1;")
    ...
    '                StrSQL = StrSQL & fItem_Inventory_SQL(.Cells("ITEM_ID").Value, cmbLOCATION_ID.SelectedValue, THIS_QTY, LAST_ENDING_UNIT_COST, 12, NumIsNull(.Cells("ID").Value), dtpDATE.Value)
    '            End If
    ...
    '                StrSQL = StrSQL & fAccount_Journal_SQL(.Cells("INCOME_ACCOUNT_ID").Value, cmbLOCATION_ID.SelectedValue, .Cells("ITEM_ID").Value, 14, NumIsNull(.Cells("ID").Value), dtpDATE.Value, E, AMT, gsJOURNAL_NO_FORM)
    ...
    '                    'ASSET_ACCOUNT_ID
    '                    StrSQL = StrSQL & fAccount_Journal_SQL(.Cells("ASSET_ACCOUNT_ID").Value, cmbLOCATION_ID.SelectedValue, .Cells("ITEM_ID").Value, 14, NumIsNull(.Cells("ID").Value), dtpDATE.Value, 0, AMT_COST, gsJOURNAL_NO_FORM)
    '                    'COGS_ACCOUNT_ID
    '                    StrSQL = StrSQL & fAccount_Journal_SQL(.Cells("COGS_ACCOUNT_ID").Value, cmbLOCATION_ID.SelectedValue, .Cells("ITEM_ID").Value, 14, NumIsNull(.Cells("ID").Value), dtpDATE.Value, 1, AMT_COST, gsJOURNAL_NO_FORM)
            ...
            If MessageBoxQuestion(gsMessageQuestion) = True Then
                ...
                Try
                    ...
                    While rd.Read
                        ...
                        fInvoiceBalance_Update(rd("INVOICE_ID"), GetNumberFieldValue("INVOICE", "ID", rd("INVOICE_ID"), "AMOUNT"))
                ...
                If gsSkipJournalEntry = False Then
                    ...
                    fAccount_journal_Delete(Val(cmbACCOUNTS_RECEIVABLE_ID.SelectedValue), cmbLOCATION_ID.SelectedValue, 12, ID, dtpDATE.Value)

                    If NumIsNull(lblOUTPUT_TAX_ACCOUNT_ID.Text) <> 0 Then
                        fAccount_journal_Delete(Val(lblOUTPUT_TAX_ACCOUNT_ID.Text), cmbLOCATION_ID.SelectedValue, 12, ID, dtpDATE.Value)
                    End If
    ...
    Private Sub ToolStripButton3_Click(sender As Object, e As EventArgs) Handles tsJournal.Click
        ...
        End If
        If IsNew = False Then
            fTransactionJournal(ID, dtpDATE.Value, cmbLOCATION_ID.SelectedValue, 12, 4, cmbCUSTOMER_ID.Text, txtCODE.Text, txtNOTES.Text)
        End If
    End Sub
