...
Public Class FrmSalesTaxRecompute
    Private Sub FrmSalesTaxRecompute_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        fComboBox(cmbITEM_ID, "select ID,DESCRIPTION from ITEM  Where TYPE ='0' and INACTIVE='0' ", "ID", "DESCRIPTION")
    ...
    Private Sub btnStart_Click(sender As Object, e As EventArgs) Handles btnStart.Click
        ...
        Dim rd As OdbcDataReader = fReader($"select * from item where id = '{cmbITEM_ID.SelectedValue}' limit 1")
        ...
        If rd.Read Then
            ...
            Else
                SQL = $"select sri.`ID`,sri.`AMOUNT`,sri.`TAXABLE`,sri.`TAXABLE_AMOUNT`,sri.`TAX_AMOUNT`,sri.`SALES_RECEIPT_ID`,sr.`OUTPUT_TAX_ID`,sr.`OUTPUT_TAX_ACCOUNT_ID`,sr.`OUTPUT_TAX_AMOUNT`,sr.`OUTPUT_TAX_RATE`,sr.`POS_LOG_ID`,sr.`DATE`,sri.INCOME_ACCOUNT_ID,sr.CUSTOMER_ID  from sales_receipt_items as sri inner join sales_receipt as sr on sr.id = sri.sales_receipt_id Where sri.`ITEM_ID`= '{cmbITEM_ID.SelectedValue}' and sr.`LOCATION_ID`  = '{cmbLOCATION_ID.SelectedValue}' and sr.DATE >='{fDateFormatMYSQL(dtpUseDate.Value)}' "
            End If
            Dim Run As Integer = 0
            Dim Count As Integer

            Dim r As OdbcDataReader = fReaderCounting(SQL, Count)
            ...
            While r.Read
                ...
                Item_amt_Value = fUpdateSalesReceiptItem(fNumisNULL(r("ID")), fNumisNULL(r("AMOUNT")), fNumisNULL(rd("TAXABLE")), fNumisNULL(r("OUTPUT_TAX_ID")))
                ...
                'ITEM
                fExecutedOnly($"UPDATE account_journal SET AMOUNT='{Item_amt_Value}' WHERE OBJECT_TYPE = '53' and OBJECT_ID = '{r("ID")}' and ACCOUNT_ID = '{r("INCOME_ACCOUNT_ID")}' and SUBSIDIARY_ID ='{r("CUSTOMER_ID")}' and LOCATION_ID = '{cmbLOCATION_ID.SelectedValue}' and `OBJECT_DATE` = '{fDateFormatMYSQL(r("DATE"))}' limit 1; ")
                'TAX
                fExecutedOnly($"UPDATE account_journal SET AMOUNT='{Out_put_Value}' WHERE OBJECT_TYPE = '52' and OBJECT_ID = '{r("SALES_RECEIPT_ID")}' and ACCOUNT_ID = '28' and SUBSIDIARY_ID ='{r("CUSTOMER_ID")}' and LOCATION_ID = '{cmbLOCATION_ID.SelectedValue}' and `OBJECT_DATE` = '{fDateFormatMYSQL(r("DATE"))}' limit 1; ")
    ...
    Private Sub fPOS_LOG_TAX_UPDATE(ByVal POS_LOG_ID As Integer)
        ...
        If rd.Read Then
            fExecutedOnly($"UPDATE pos_log SET TAXABLE_AMOUNT ='{ fNumisNULL(rd("TAXABLE_AMOUNT"))}',OUTPUT_TAX_AMOUNT='{ fNumisNULL(rd("TAX_AMOUNT"))}',NONTAXABLE_AMOUNT='{ fNumisNULL(rd("NON_TAXABLE_AMOUNT"))}'   WHERE ID = '{POS_LOG_ID}' limit 1; ")
        End If
        rd.Close()
    End Sub
    Private Function fUpdateSalesReceiptItem(ByVal ID As Integer, ByVal AMT As Double, ByVal prTaxable As Boolean, ByVal Tax_Type As Integer) As Double
        ...
        If prTaxable = True Then
            ...
            dVat = fTax_Rate_Find(fNumisNULL(Tax_Type))
        ...
        fExecutedOnly($"UPDATE sales_receipt_items set TAXABLE='{Val(prTaxable)}',TAXABLE_AMOUNT ='{ fNumFormatFixed(Taxable_Amount)}', TAX_AMOUNT = '{fNumFormatFixed(Tax_Amount)}'  WHERE id = '{ID}' limit 1;")
    ...
    Private Function UpdateSalesReceiptOnly(ByVal ID As Integer) As Double
        ...
        If rd.Read Then
            ...
            fExecutedOnly($"UPDATE sales_receipt SET OUTPUT_TAX_AMOUNT ='{ fNumisNULL(rd("TAX_AMOUNT"))}',TAXABLE_AMOUNT ='{ fNumisNULL(rd("TAXABLE_AMOUNT"))}',NONTAXABLE_AMOUNT='{ fNumisNULL(rd("NON_TAXABLE_AMOUNT"))}' WHERE ID = '{ID}' limit 1;")
            OUT_PUT_TAX = fNumisNULL(rd("TAX_AMOUNT"))
        End If
        rd.Close()
