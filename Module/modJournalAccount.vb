...
Module modJournalAccount
    ...
    Private Sub fGET_PREVIOUS_ACCOUNT(ByVal prAccount_ID As String, ByVal prsubsidiary_id As String, ByVal prLocation_ID As String, ByRef prSEQUENCE_No As Integer, ByRef prID As Double, ByRef prENDING_BALANCE As Double, ByVal ThisID As Integer)
        ...
        Try
            ...
            If rd.Read Then
                ...
                prSEQUENCE_No = fNumisNULL(rd("SEQUENCE_NO"))
                prENDING_BALANCE = fNumisNULL(rd("ENDING_BALANCE"))
            Else
                prID = 0
                prSEQUENCE_No = 0
                prENDING_BALANCE = 0
    ...
    Public Sub fPOS_STARTING_CASH_JOURNAL_Delete(ByVal prSTART_ID As String, ByVal prDATE As Date, ByVal prLOCATION_ID As Integer)
        Dim prOBJECT_TYPE_ITEM_ID As String = fGetObjectTypeMap_ID("POS_STARTING_CASH")
        ...
        If rd.Read Then
            sID = fNumisNULL(prSTART_ID) 'POS STARTING CASH
            ...
            Dim SUBSIDAIRY_ID As Integer = fNumisNULL(rd("POS_MACHINE_ID"))
    ...
    Public Sub fPOS_STARTING_CASH_JOURNAL(ByVal prSTART_ID As String, ByVal prDATE As Date, ByVal prLOCATION_ID As Integer)
        ...
        If rd.Read Then
            ...
            Dim sAMOUNT As Double = fNumisNULL(rd("AMOUNT"))

            sID = fNumisNULL(prSTART_ID) 'STARTING CASH ID
            Dim xID As Double = 0
            Dim sACCOUNT_DRAWER_ID As Integer = Val(rd("DRAWER_ACCOUNT_ID"))
            Dim sACCOUNT_PETTY_CASH_ID As Integer = Val(rd("PETTY_CASH_ACCOUNT_ID"))
            ...
            If dJOURNAL_NO = 0 Then
                If primary_journal_no = 0 Then
                    primary_journal_no = fNumisNULL(fGetMaxField("JOURNAL_NO", "ACCOUNT_JOURNAL"))
    ...
    Public Sub fPOS_LOG_JOURNAL(ByVal prPOS_ID As String, ByVal DS_ACCOUNT_ID As Integer, ByVal prDATE As Date)
        ...
        Dim prOBJECT_TYPE_ITEM_ID As String = fGetObjectTypeMap_ID("POS_LOG")
        Dim dJOURNAL_NO As Integer = 0
        Dim primary_journal_no As Integer = 0
        ...
        If rd.Read Then
            ...
            Dim LOG_TOTAL As Double = fNumisNULL(rd("LOG_TOTAL"))
            Dim COUNT_TOTAL As Double = fNumisNULL(rd("COUNT_TOTAL"))
            Dim DS_TOTAL As Double = fNumisNULL(rd("DS_TOTAL"))

            sID = fNumisNULL(prPOS_ID) 'POS LOG ID
            Dim xID As Double = 0
            Dim sACCOUNT_DRAWER_ID As Integer = Val(rd("DRAWER_ACCOUNT_ID"))
    ...
    Private Sub fAccount_Journal(ByRef ID As Double, ByVal PREVIOUS_ID As Double, ByVal SEQUENCE_NO As Double,
        ...
        If ID = 0 Then
            ...
            Dim xID As Double = fObjectTypeMap_ID("ACCOUNT_JOURNAL")

            fExecutedOnly("INSERT INTO account_journal  SET ID = '" & xID & "', PREVIOUS_ID =" & fGotNullNumber(PREVIOUS_ID) & ",SEQUENCE_NO = '" & SEQUENCE_NO & "',JOURNAL_NO = '" & JOURNAL_NO & "',
            ...
            OBJECT_DATE = '" & OBJECT_DATE & "',ENTRY_TYPE = '" & ENTRY_TYPE & "',AMOUNT = '" & AMOUNT & "',ENDING_BALANCE ='" & dBALANCE & "',EXTENDED_OPTIONS = " & fGotNullText(EXTENDED_OPTIONS.Replace("INVENTORY", "")))

            fFixed_Ending_Balance(OBJECT_DATE, PREVIOUS_ID, ACCOUNT_ID, LOCATION_ID)

        Else
            ' When Amount is not Change the Update not procceed
            If fNumFieldValueByTwoCondition("ACCOUNT_JOURNAL", "ID", ID, "ACCOUNT_ID", ACCOUNT_ID, "Amount") <> AMOUNT Then

                fExecutedOnly("UPDATE account_journal  SET PREVIOUS_ID =" & fGotNullNumber(PREVIOUS_ID) & ",SEQUENCE_NO = '" & SEQUENCE_NO & "',JOURNAL_NO = '" & JOURNAL_NO & "',
                ACCOUNT_ID = '" & ACCOUNT_ID & "',LOCATION_ID = '" & LOCATION_ID & "',SUBSIDIARY_ID='" & SUBSIDIARY_ID & "',
                SEQUENCE_GROUP = '" & SEQUENCE_GROUP & "',OBJECT_TYPE = '" & OBJECT_TYPE & "',OBJECT_ID = '" & OBJECT_ID & "',
                OBJECT_DATE = '" & OBJECT_DATE & "',ENTRY_TYPE = '" & ENTRY_TYPE & "',AMOUNT = '" & AMOUNT & "',ENDING_BALANCE ='" & dBALANCE & "',EXTENDED_OPTIONS = " & fGotNullText(EXTENDED_OPTIONS.Replace("INVENTORY", "")) & " WHERE ID = '" & ID & "' limit 1")
            ...
            Dim rdc As OdbcDataReader = fReader("SELECT * FROM account_journal AS j WHERE j.JOURNAL_NO <> '" & JOURNAL_NO & "'  AND j.`OBJECT_TYPE` = '" & OBJECT_TYPE & "' AND j.`OBJECT_ID` ='" & OBJECT_ID & "' AND j.`ENTRY_TYPE` = '" & ENTRY_TYPE & "' AND j.`OBJECT_DATE` = '" & OBJECT_DATE & "' AND account_ID ='" & ACCOUNT_ID & "' AND J.LOCATION_ID = '" & LOCATION_ID & "' Limit 1")
            If rdc.Read Then
                If fMessageboxWarningYesNo($"Duplicate Journal No. {fTextisNULL(rdc("JOURNAL_NO"))} found do you want to continue?") = True Then
                    'Doing to remove the duplicate
                    fJournalAccountRemoveFixed_Account_ID_SP(fNumisNULL(rdc("ACCOUNT_ID")), fNumisNULL(rdc("OBJECT_TYPE")), fNumisNULL(rdc("OBJECT_ID")), rdc("OBJECT_DATE"), LOCATION_ID, fNumisNULL(rdc("SUBSIDIARY_ID")), rdc("ID"))
                End If
            End If
    ...
    Private Sub fFixed_Ending_Balance(ByVal OBJECT_DATE As Date, ByVal PREVIOUS_ID As Integer, ByVal ACCOUNT_ID As Integer, ByVal LOCATION_ID As Integer)
        ...
        If OBJECT_DATE < DateTime.Now.Date Then
            ...
            Dim rd As OdbcDataReader = fReader(sql_Fix)

            While rd.Read
                If PREVIOUS_ID <> fNumisNULL(rd("ID")) Then

                    If fNumisNULL(rd("ENTRY_TYPE")) = 0 Then
                        T_ENDING_BALANCE = T_ENDING_BALANCE + fNumisNULL(rd("AMOUNT"))
                    Else
                        T_ENDING_BALANCE = T_ENDING_BALANCE - fNumisNULL(rd("AMOUNT"))
                    End If

                    fExecutedOnly($"UPDATE account_journal SET  PREVIOUS_ID ={fGotNullNumber(T_PREVIOUS_ID)},ENDING_BALANCE = '{T_ENDING_BALANCE}' WHERE ID ='{rd("ID")}' and ACCOUNT_ID ='{ACCOUNT_ID}' and LOCATION_ID='{LOCATION_ID}'  limit 1;")
                    T_PREVIOUS_ID = rd("ID")
                Else
                    T_ENDING_BALANCE = fNumisNULL(rd("ENDING_BALANCE"))
                    T_PREVIOUS_ID = rd("ID")
                End If
        ...
        Try
            ...
            If rd.Read Then
                ...
                Else
                    ...
                    prSEQUENCE_No = fNumFieldValueByTwoCondition("ITEM_INVENTORY", "ITEM_ID", prITEM_ID, "LOCATION_ID", prLocation_ID, "MAX(SEQUENCE_NO)")

                End If

                If prDATE >= dt Then
                    If prENDING_UNIT_COST = 0 Then
                        prENDING_UNIT_COST = fNumisNULL(rd("ENDING_UNIT_COST"))
                    Else
                        prENDING_UNIT_COST = prENDING_UNIT_COST
                    End If
                    prENDING_QTY = fNumisNULL(rd("ENDING_QUANTITY"))
                    prENDING_COST = fNumisNULL(rd("ENDING_COST"))
                End If
            Else
                prID = 0
                If prSEQUENCE_No <> 0 Then
    ...
    Private Function fGET_TRANSACTION_JOURNAL(ByVal prACCOUNT_ID As String, ByVal prOBJECT_TYPE As String, ByVal prOBJECT_ID As String, ByVal prLocation_ID As String, ByVal prENTRY_TYPE As Integer, ByVal prDATE As String, ByRef prPREVIOUS_ID As Double, ByRef prSEQUENCE_NO As Integer, ByRef prJOURNAL_NO As Double) As Double
        ...
        Try
            ...
            If rd.Read Then
                ...
                prPREVIOUS_ID = fNumisNULL(rd("PREVIOUS_ID"))
                prSEQUENCE_NO = fNumisNULL(rd("SEQUENCE_NO"))
                prJOURNAL_NO = fNumisNULL(rd("JOURNAL_NO"))
            Else
                prPREVIOUS_ID = 0
                prSEQUENCE_NO = 0
                prJOURNAL_NO = 0
    ...
    Private Sub fReConfigAccountJournal_ENDING_BALANCE(ByVal prACCOUNT_ID As Integer, ByVal prLocation_ID As Integer, ByVal prSUBSIDIARY_ID As Double, ByVal prSEQUENCE_NO As Double, ByVal prSEQUENCE_GROUP As Double, ByVal prID As Double)
        ...
        Try
            ...
            Dim rd As OdbcDataReader = fReader(sql_select)
            While rd.Read
                If bFirst = False Then
                    dEnding_Balance = fNumisNULL(rd("ENDING_BALANCE"))
                Else
                    If fNumisNULL(rd("ENTRY_TYPE")) = 0 Then
                        dAmount = fNumisNULL(rd("AMOUNT"))
                        dEnding_Balance = dEnding_Balance + dAmount
                    Else
                        dAmount = fNumisNULL(rd("AMOUNT"))
                        dEnding_Balance = dEnding_Balance - dAmount
                    End If
                    sQuery = sQuery & fAccount_Journal_Collect_SQL(rd("ID"), fNumisNULL(rd("PREVIOUS_ID")), prACCOUNT_ID, fNumisNULL(rd("SEQUENCE_NO")), prSEQUENCE_GROUP, prSUBSIDIARY_ID, prLocation_ID, dEnding_Balance)
                End If
                bFirst = True
    ...
    Private Sub fReConfigItemInventory_ENDING_Balance(ByVal prItem_ID As Double, ByVal prLocation_ID As Integer, ByVal prSEQUENCE_NO As Double, ByVal prID As Double)
        ...
        Try
            ...
            Dim rd As OdbcDataReader = fReader(sSelect_SQL)
            While rd.Read
                If bFirst = False Then
                    LAST_ENDING_QTY = fNumisNULL(rd("ENDING_QUANTITY"))
                Else
                    LAST_ENDING_QTY = LAST_ENDING_QTY + fNumisNULL(rd("QUANTITY"))
                    dENDING_COST = LAST_ENDING_QTY * fNumisNULL(rd("ENDING_UNIT_COST"))
    ...
    Public Sub fItemInventoryRemoveFixedByType(ByVal prSOURCE_TYPE As Integer, ByVal prSOURCE_ID As Double, ByVal prSOURCE_DATE As Date, ByVal prItem_ID As Double, ByVal prLocation_ID As Integer)
        ...
        Try
            Dim rd As OdbcDataReader = fReader(sItem_Inventory)
            If rd.Read Then
                bOpen = True
                prID = fNumisNULL(rd("ID"))
                prPrev_ID = fNumisNULL(rd("PREVIOUS_ID"))
                prSEQUENCE_NO = fNumisNULL(rd("SEQUENCE_NO"))
            End If
            rd.Close()
            rd = Nothing
            If bOpen = True Then
                ...
                Dim sQuery As String = ""
                rd = fReader(sSelect_SQL)
                While rd.Read
                    If prSEQUENCE_NO = fNumisNULL(rd("SEQUENCE_NO")) And prID = fNumisNULL(rd("ID")) Then
                    ...
                    Else
                        ENDING_QUANTITY = fNumisNULL(rd("ENDING_QUANTITY")) + fNumisNULL(rd("QUANTITY"))
                        sQuery = "UPDATE item_inventory SET ENDING_COST='" & fNumisNULL(rd("ENDING_UNIT_COST")) * ENDING_QUANTITY & "',ENDING_QUANTITY='" & ENDING_QUANTITY & "',PREVIOUS_ID = " & fGotNullNumber(prPrev_ID) & " WHERE ID = '" & rd("ID") & "' and ITEM_ID = '" & prItem_ID & "' and LOCATION_ID='" & prLocation_ID & "';" & sQuery
                        prPrev_ID = fNumisNULL(rd("ID"))
                    End If
    ...
    Public Sub fJournalAccountRemoveFixed(ByVal prTable_Name As String, ByVal prOBJECT_ID As String, ByVal prOBJECT_DATE As Date, ByVal prLocation_ID As Integer, ByVal prSUBSIDIARY_ID As Integer)
        Dim iObj_Type As Integer = fGetFieldValue("object_type_map", "TABLE_NAME", prTable_Name, "ID")

        Try
            Dim rd As OdbcDataReader = fReader("select ID,SEQUENCE_NO,SUBSIDIARY_ID,ACCOUNT_ID,SEQUENCE_GROUP from account_journal where OBJECT_TYPE = '" & iObj_Type & "' and OBJECT_ID = '" & prOBJECT_ID & "' and OBJECT_DATE = '" & Format(prOBJECT_DATE, "yyyy-MM-dd") & "' and Location_ID = '" & prLocation_ID & "' and SUBSIDIARY_ID = '" & prSUBSIDIARY_ID & "'")
            While rd.Read
    ...
    Private Sub fJournal_Account_Execute(ByVal prSEQUENCE_NO As Integer, ByVal prSUBSIDIARY_ID As Integer, ByVal prACCOUNT_ID As Integer, ByVal prLocation_ID As Integer, prSEQUENCE_GROUP As Integer, ByVal prID As Double)
        ...
        Try
            ...
            While rd.Read
                ...
                If prSEQUENCE_NO = rd("SEQUENCE_NO") And prID = fNumisNULL(rd("ID")) Then
                    ENTRY_TYPE = fNumisNULL(rd("ENTRY_TYPE"))
                    AMOUNT = fNumisNULL(rd("AMOUNT"))
                    If ENTRY_TYPE = 0 Then
                        ENDING_BALANCE = fNumisNULL(rd("ENDING_BALANCE")) - AMOUNT
                    Else
                        ENDING_BALANCE = fNumisNULL(rd("ENDING_BALANCE")) + AMOUNT
                    End If
                    dPrevious_ID = fNumisNULL(rd("PREVIOUS_ID"))
                    fExecutedOnly($"UPDATE account_journal SET PREVIOUS_ID = NULL WHERE ID = '{prID}' limit 1;")
                    fExecutedOnly($"UPDATE account_journal SET PREVIOUS_ID = {fGotNullNumber(dPrevious_ID)} WHERE PREVIOUS_ID = '{prID}' limit 1;")
                    fExecutedOnly($"DELETE FROM account_journal WHERE ID ='" & prID & "' and SEQUENCE_NO = '" & fNumisNULL(rd("SEQUENCE_NO")) & "' and SUBSIDIARY_ID ='" & prSUBSIDIARY_ID & "' and ACCOUNT_ID = '" & prACCOUNT_ID & "' and Location_ID = '" & prLocation_ID & "' and SEQUENCE_GROUP = '" & prSEQUENCE_GROUP & "' limit 1;")
                ...
                Else
                    ...
                    If sQuery <> "" Then
                        ...
                        Else
                            ENDING_BALANCE = ENDING_BALANCE - AMOUNT
                        End If
                        fExecutedOnly("UPDATE account_journal SET ENDING_BALANCE='" & ENDING_BALANCE & "',PREVIOUS_ID =" & fGotNullNumber(dPrevious_ID) & " WHERE ID ='" & rd("ID") & "' and SEQUENCE_NO = '" & fNumisNULL(rd("SEQUENCE_NO")) & "' and SUBSIDIARY_ID ='" & prSUBSIDIARY_ID & "' and ACCOUNT_ID = '" & prACCOUNT_ID & "' and Location_ID = '" & prLocation_ID & "' and SEQUENCE_GROUP = '" & prSEQUENCE_GROUP & "' limit 1;")
                        dPrevious_ID = fNumisNULL(rd("ID"))
                    End If
                End If
            End While
            rd.Close()
    ...
    Public Sub fJournalAccountRemoveFixed_Account_ID(ByVal prAccount_ID As Integer, ByVal iObj_Type As Integer, ByVal prOBJECT_ID As String, ByVal prOBJECT_DATE As Date, ByVal prLocation_ID As Integer, ByVal prSUBSIDIARY_ID As Integer)
        ...
        Try
            ...
            If rd.Read Then
                fJournal_Account_Execute(fNumisNULL(rd("SEQUENCE_NO")), fNumisNULL(rd("SUBSIDIARY_ID")), fNumisNULL(rd("ACCOUNT_ID")), prLocation_ID, rd("SEQUENCE_GROUP"), rd("ID"))
            End If
            rd.Close()
        Catch ex As Exception
            fMessageboxWarning(ex.Message)
