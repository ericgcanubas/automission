...
Module modInventoryItem
    ...
    Public Sub fBILL_ITEM_COST_UPDATE_NEW(ByVal prItem_ID As String, ByVal prCOST As Double)
        fExecutedOnly("UPDATE item SET COST = '" & prCOST & "' where ID = '" & prItem_ID & "' Limit 1;")
    End Sub
    ...
    Private Function fLatestAdjustment(ByVal prITEM_ID As Integer, ByVal prLOCATION_ID As Integer, ByVal prSOURCE_REF_ID As Integer, ByVal prSOURCE_REF_TYPE As Integer) As Boolean
        Dim B As Boolean
        Dim rd As OdbcDataReader = fReader($"SELECT i.SOURCE_REF_DATE,i.QUANTITY,i.ENDING_QUANTITY,i.COST,i.ENDING_UNIT_COST,i.ENDING_COST FROM item_inventory  AS i WHERE i.`ITEM_ID` = '{prITEM_ID}' AND i.`LOCATION_ID` = '{prLOCATION_ID}' AND i.SOURCE_REF_TYPE ='{prSOURCE_REF_TYPE}' and i.SOURCE_REF_ID = '{prSOURCE_REF_ID}' order by i.ID desc Limit 1 ")
        If rd.Read Then
    ...
    Public Sub fReCalculateInventory(ByVal prITEM_ID As Integer, ByVal prLOCATION_ID As Integer, ByVal ThisDate As Date)
        fCursorLoadingOn(True)
        Dim Qty_END_T As Double = fQTY_ACTUAL_ON_DATE_LOCATION(prITEM_ID, ThisDate, prLOCATION_ID)

        Dim NewSQL As String = ""

        Dim rd As OdbcDataReader = fReader($"SELECT i.* FROM item_inventory  AS i WHERE i.SOURCE_REF_DATE >='{ fDateFormatMYSQL(ThisDate) }' and i.`ITEM_ID` = '{prITEM_ID}' AND i.`LOCATION_ID` = '{prLOCATION_ID}' order by i.SOURCE_REF_DATE,i.ID")
        While rd.Read
            If fNumisNULL(rd("SOURCE_REF_TYPE")) = 6 Then
                Dim L_QTY As Double = Qty_END_T
                Qty_END_T = fNumisNULL(rd("ENDING_QUANTITY"))
                Dim D_QTY As Double = Qty_END_T - L_QTY
                NewSQL = NewSQL & $"UPDATE item_inventory SET QUANTITY='{fNumisNULL(D_QTY)}' WHERE ID ='{rd("ID")}' and ITEM_ID = '{prITEM_ID}' and LOCATION_ID ='{prLOCATION_ID}' Limit 1;"
            Else
                Qty_END_T = Qty_END_T + fNumisNULL(rd("QUANTITY"))
                NewSQL = NewSQL & $"UPDATE item_inventory SET ENDING_QUANTITY='{fNumisNULL(Qty_END_T)}' WHERE ID ='{rd("ID")}' and ITEM_ID = '{prITEM_ID}' and LOCATION_ID ='{prLOCATION_ID}' Limit 1;"
            End If

        End While
        rd.Close()
        If NewSQL <> "" Then
            fExecutedOnly(NewSQL)
    ...
    Public Sub fDeleteItem_INVENTORY_ITEM_RECALCULATE(ByVal dgv As DataGridView, ByVal LOCATION_ID As Integer, ByVal SOURCE_DATE As Date)
        ...
        For I As Integer = 0 To dgv.Rows.Count - 1
            ...
            fReCalculateInventory(ITEM_ID, LOCATION_ID, SOURCE_DATE.AddDays(-1))
    ...
    Public Sub fUpdateItemInventory_AccountJournalCost(ByVal prITEM_ID As Integer, ByVal prLOCATION_ID As Integer, ByVal prSOURCE_REF_ID As Integer, ByVal prSOURCE_REF_TYPE As Integer, ByVal prOBJECT_TYPE As Integer)
        If fLatestAdjustment(prITEM_ID, prLOCATION_ID, prSOURCE_REF_ID, prSOURCE_REF_TYPE) = True Then
            ...
            Dim rd As OdbcDataReader = fReader($"SELECT i.* FROM item_inventory  AS i WHERE i.SOURCE_REF_DATE >='{ fDateFormatMYSQL(DateStart) }' and i.`ITEM_ID` = '{prITEM_ID}' AND i.`LOCATION_ID` = '{prLOCATION_ID}' AND i.SOURCE_REF_TYPE <> '{prSOURCE_REF_TYPE}' order by i.ID")
            While rd.Read
                Qty_END_T = Qty_END_T + fNumisNULL(rd("QUANTITY"))
                Cost_UN_END = Ending_unit_cost

                If fNumisNULL(rd("QUANTITY")) <= 0 Then
                    COST_T = 0
                ...
                fExecutedOnly($"UPDATE item_inventory SET ENDING_QUANTITY='{fNumisNULL(Qty_END_T)}',COST={fGotNullNumber(COST_T)},ENDING_COST={fGotNullNumber(Temp_ENDING_COST)},ENDING_UNIT_COST={fGotNullNumber(Cost_UN_END)} WHERE ID ='{rd("ID")}' and ITEM_ID = '{prITEM_ID}' and LOCATION_ID ='{prLOCATION_ID}' Limit 1;")

            End While
            rd.Close()

            Dim item_rd As OdbcDataReader = fReader($"Select COGS_ACCOUNT_ID,ASSET_ACCOUNT_ID from item Where ID = '{prITEM_ID}'  Limit 1")
    ...
    Private Sub fCostUpdateJournal(ByVal ACCOUNT_ID As Integer, ByVal ITEM_SUBSI As Integer, ByVal LOCATION_ID As Integer, ByVal ThisCost As Double, ByVal ThisDate As Date, ByVal prOBJECT_TYPE As Integer)
        ...
        Dim rd As OdbcDataReader = fReader($"SELECT * FROM account_journal WHERE OBJECT_TYPE <> '{prOBJECT_TYPE}' and SUBSIDIARY_ID = '{ITEM_SUBSI}' AND location_id ='{LOCATION_ID}' AND account_id = '{ACCOUNT_ID}' and OBJECT_DATE >='{fDateFormatMYSQL(ThisDate)}' ORDER BY OBJECT_DATE,ID")
        While rd.Read

            Dim SQL_ITEM As String
            If 20 = fNumisNULL(rd("OBJECT_TYPE")) Then
            ...
            Else
                SQL_ITEM = $"SELECT QUANTITY FROM {fGetFieldValue("OBJECT_TYPE_MAP", "ID", rd("OBJECT_TYPE"), "TABLE_NAME")} WHERE ID='{rd("OBJECT_ID")}'  and ITEM_ID ='{ITEM_SUBSI}' Limit 1; "

            End If

            Dim rd_item As OdbcDataReader = fReader(SQL_ITEM)
            If rd_item.Read Then
                If fNumisNULL(rd("ENTRY_TYPE")) = 0 Then
                    T_Balance = T_Balance + (ThisCost * fNumisNULL(rd_item("QUANTITY")))
                Else
                    T_Balance = T_Balance - (ThisCost * fNumisNULL(rd_item("QUANTITY")))
                End If

                Tmp_SQL = Tmp_SQL & $"UPDATE account_journal SET AMOUNT = '{(ThisCost * fNumisNULL(rd_item("QUANTITY")))}', ENDING_BALANCE ='{T_Balance}' WHERE ID ='{rd("ID")}' and SUBSIDIARY_ID = '{ITEM_SUBSI}' AND location_id ='{LOCATION_ID}' AND account_id = '{ACCOUNT_ID}' Limit 1;"
            End If
            rd_item.Close()
        End While
        rd.Close()
    ...
    Private Function fGET_ENDING_BALANCE(ByVal ACCOUNT_ID As Integer, ByVal ITEM_SUBSI As Integer, ByVal LOCATION_ID As Integer, ByVal ThisDate As Date) As Double
        ...
        Dim rd As OdbcDataReader = fReader($"SELECT ENDING_BALANCE FROM account_journal WHERE SUBSIDIARY_ID = '{ITEM_SUBSI}' AND location_id ='{LOCATION_ID}' AND account_id = '{ACCOUNT_ID}' and OBJECT_DATE < '{fDateFormatMYSQL(ThisDate)}' ORDER BY OBJECT_DATE DESC,ID DESC LIMIT 1")
        If rd.Read Then
            ThisBalance = fNumisNULL(rd("ENDING_BALANCE"))
        End If
        rd.Close()
        Return ThisBalance
    End Function
    Public Sub InventoryVDetailsQuickView(ByVal prITEM_ID As Integer, ByVal prLOCATION_ID As Integer, ByVal dtSTART As Date, ByVal prFind As String)
        ...
        fCryParameterInsertValue(gscryRpt, fDateFormatMYSQL(dtSTART), "fdate")
        fCryParameterInsertValue(gscryRpt, fDateNow, "tdate")
        ...
        fCryParameterInsertValue(gscryRpt, fDateFormatStandard(fDateNow), "date_remark")
        ...
        frmReportViewer.Text = "Item Valuation Details " & fDateTimeNow()
        If prFind <> "" Then
            '  frmReportViewer.CrystalReportViewer1.f
