...
Module modQuantityOnHand
    ...
    Public Function fQTY_ACTUAL_ON_DATE_LOCATION(ByVal prItem_ID As Integer, ByVal dt As Date, ByVal sLocation_ID As Integer) As Double

        Dim sON_HAND As String

        If sLocation_ID <= 0 Then
        ...
        Else
            sON_HAND = "(SELECT format( ix.`ENDING_QUANTITY`,0) FROM item_inventory AS ix WHERE ix.item_id = i.ID AND ix.`LOCATION_ID` = '" & sLocation_ID & "' AND ix.SOURCE_REF_DATE <= '" & Format(dt, "yyyy-MM-dd") & "' ORDER BY ix.`SOURCE_REF_DATE` DESC,ix.ID DESC LIMIT 1 ) "
        End If
