...
Module modInvoice_payment
    ...
    Public Function fGetSumPaymentApplied(ByRef prInvoice_ID As String, ByVal prCustomer_ID As String) As Double
        ...
        Try
            Dim rd As OdbcDataReader = fReader("select SUM(pv.AMOUNT_APPLIED)  as P, SUM(pv.DISCOUNT) as D from payment_invoices as pv  inner join payment as p on p.id = pv.payment_id  where p.customer_id = '" & prCustomer_ID & "' and pv.invoice_id = '" & prInvoice_ID & "' and p.STATUS ='15'")
            If rd.Read Then
    ...
    Public Function fGetSumCreditApplied(ByRef prInvoice_ID As String, ByVal prCustomer_ID As String) As Double
        ...
        Try
            ...
            Dim rd As OdbcDataReader = fReader("select SUM(cmi.AMOUNT_APPLIED)  as P  from credit_memo_invoices as cmi inner join credit_memo as c on c.id = cmi.credit_memo_id where c.customer_id = '" & prCustomer_ID & "' and cmi.invoice_id = '" & prInvoice_ID & "'")
            If rd.Read Then
